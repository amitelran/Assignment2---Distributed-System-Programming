...
public class WordCount { 
	...
	public static class Mapper_A extends Mapper<TweetKey, TweetValue, Text, TupleWritable> {
		...
		// From Mapper_A: KEYIN = word (Text), VALUEIN = tuple (TweetKey, TweetValue, number of times word appeared in tweet, maximal number of times any word appeared in the tweet)
		...
		public static class Partitioner_A extends Partitioner<Text, IntWritable> {

			// Get the partition number for a given key (hence record) given the total number of partitions ( = total number of reducers)
			@Override
			public int getPartition(Text word, VectorCorpus tweetVector, int numPartitions) {
